...
- 최종 판정 요약, 충족·미충족·추가 확인 조건과 공식 출처는 LLM 응답이 아니라 구조화된 입력에서 조립한다.
...
- LLM 자유 서술은 정책 설명으로 노출하지 않고, 검증된 출력이 실제 선택한 Citation 원문 조각만 서버에서 조립한다.
- Rule 조건 ID의 필수·전역 유일성과 질문·조건·Citation·그래프 노드의 개수 및 길이 제한을 검증한다.
- Provider가 fallback을 표시하면 `GROUNDED`로 취급하지 않고 서버의 `SAFE_FALLBACK`으로 교체한다.
- 정책 설명과 일반 안내를 `policyExplanation`, `generalGuidance`로 분리했다.

## 제한사항

- A5는 설명 생성 모듈만 제공하며 HTTP API에는 아직 연결하지 않는다.
- 반대 문장 검사는 보조 방어선이다. LLM 자유 서술은 응답에 노출하지 않으며 최종 판정과 조건은 항상 결정론적 필드가 권위값이다.
