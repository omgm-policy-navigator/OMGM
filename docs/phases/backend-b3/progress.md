...
- Added server-generated UUID4 anonymous session tokens transported only in the `anonymous_session` HttpOnly cookie.
...
- Added Origin validation for cookie-authenticated unsafe methods and rejected client-supplied session IDs in JSON or session headers.
...
- Added B3 tests for cookie-only session identity, rejected client-supplied JSON/header session IDs, UUID4 token generation, idle-timeout refresh, expired-session rejection, fact scoping, cleanup delegation, and cascade relationship.
