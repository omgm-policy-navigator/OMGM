...
- Added `app/db/session.py` with async engine/session factory, explicit pool settings, `get_db()` close handling, database URL normalization, DB ping, and disposal helpers.
- Wired database configuration and disposal into FastAPI lifespan.
- Added `GET /health/live` liveness and `GET /health/ready` readiness APIs backed by a PostgreSQL `select 1` check.
- Added Alembic configuration and an initial baseline migration that enables the pgvector `vector` extension.
...
- Concrete SQLAlchemy models and table-creating schema migrations are deferred to the data-model phase.
