...
- PR #28 리뷰에 따라 승인 리포트와 Seed Patch를 결합하고, 무관한 정책·Rule·Chunk 수정과 빈 재생성을 차단했다.
- HTML/PDF extractor를 필수화하고 extraction 상태·버전을 분리했으며 HTTPS·공식 호스트 allowlist를 적용했다.
- 재생성 manifest에 report ID, 기준/생성 Seed hash, 적용 행을 기록한다.
