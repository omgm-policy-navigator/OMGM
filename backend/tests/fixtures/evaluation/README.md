...
- CI 소비자는 알 수 없는 필드를 허용하더라도 `schemaVersion` 형식이 잘못됐거나 major가 다르면 실패해야 한다. 같은 major의 minor 변경은 호환 가능하다.
...
- `e2e-scenarios.json`: 실행 가능한 Rule·Fact·RAG Case ID를 연결해 질문부터 설명까지 구성한 기대 결과
